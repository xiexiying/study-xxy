...
* Linux系统构成
  ...
  * 应用软件：包含桌面系统和基础的软件操作工具等。
  ...
  <img src="img/linux.jpg" alt="Linux" style="zoom:80%;" />
...
* 绝对路径和相对路径表达
  ...
  * 相对路径：指相对于用户当前位置为起始点，对一个文件位置的逐层描述。例如，用户处在usr目录中时，只需要“games/gnect”就可确定这个文件。在相对路径描述时  .  表示当前目录,   ..  表示上一级目录。当最后一级是普通文件，不需要加/,如果是文件夹，加不加/都可。
...
* 命令格式 
  ...
  [-options]：命令选项，辅助命令进行功能细化，也可以省略,一般以-引出
...
![image-20201030152526483](img/image-20201030152526483.png)
...
* 部分命令细节说明
  ...
  * ![image-20201030164250936](img/image-20201030164250936.png)
  * ![image-20201030164552390](img/image-20201030164552390.png)
  * wc : -c 表示查看多少字符，-l查看多少行（以换行符计算行数），-w 查看多少单词（以空格分割计算单词数）。如果不加选项则显示这三项。
  * ![image-20201030165533380](img/image-20201030165533380.png)
  * find：会从指定目录及其所有子目录中查询搜索文件。![image-20201030165121051](img/image-20201030165121051.png)
...
* 部分命令细节说明
  * zip： 用于常与windows交互的情况，-r选项可以压缩文件夹及多个文件夹
  
    * > zip    test.zip   filelist （文件列表）
      >
      > ![image-20201030173857696](img/image-20201030173857696.png)
  
    * > unzip  test.zip
      >
      > ![image-20201030174404216](img/image-20201030174404216.png)
  
  * gzip，bzip2：不常用，因为压缩或者解压后源文件就不再了，而且只能对一个文件操作，一次只能压缩一个文件
  
  * *tar：*-cjf 用于压缩bz2格式文件，-czf用于压缩gz格式文件，也可压缩多个文件
  
    -**xvf**用于解压文件,兼容了gzip和bzip2命令的功能。（所有格式）
  
    * > tar -czf   file.tar.gz   file1  file2 
      >
      > ![image-20201030175410428](img/image-20201030175410428.png)
    
    * > tar -xvf file.tar.gz
      >
      > ![image-20201030175944321](img/image-20201030175944321.png)
...
| 序号 | 命令                  | 作用                                                 |
| :--- | :-------------------  | :--------------------------------------------------- |
| 01   | sudo | 放在一个命令前，表示使用管理员权限执行(修改根目录文件) |
| 02   | chmod  | 修改文件权限       |
...
* 部分命令细节说明
  ...
  * `chmod` 在设置权限时，可以字母也可以使用三个数字分别对应 **拥有者** ／ **组** 和 **其他** 用户的权限
  ...
  chmod  augo+/-rwx(读、写、执行) 文件名/目录名#（all user greap others）增加或减少权限
  #eg：小组内其他成员没有写权限：chmod g-w
  #eg: 所有人增加写权限：chmod a+w
  ```
  
  ![image-20201102095147817](img/image-20201102095147817.png)
  ...
  > `644` ===> `u=rw,g=r,o=r`
  >
  > ![image-20201102095523032](img/image-20201102095523032.png)
...
* 部分命令细节说明
  ...
  * ![image-20201102101738751](img/image-20201102101738751.png)
  
* 输出重定向**(把原本输出终端的内容输出到文件)
...
![image-20201030170233423](img/image-20201030170233423.png)

与wc一起，可查看文件个数

![image-20201030171106955](img/image-20201030171106955.png)
...
* 部分命令细节说明
  ...
  * ln : 一般使用  -s 选项 创建软链接，相当于快捷方式，如果跨目录创建要使用绝对路径。
    ...
    ![image-20201102104118604](img/image-20201102104118604.png)
...
<img src="./img/ms.png" style="zoom: 67%;" />
...
|  r  | 替换当前字符           | 命令模式 |
|  R   | 替换当前行光标后的字符 | 替换模式 |
|  :%s/str/replace/g   | 替换str为replace               #eg:(:%s/gg/shi/g将gg替换为shi) | 底行模式 |
...
* root用户：Linux系统中的root用户通常用于系统的维护和管理，对操作系统的所有资源具有所有访问权限，一般工作中不会使用root用户进行系统操作，防止一些误操作带来系统损坏。(权限高于sudo)
...
* useradd : -m 表示添加用户时添加主目录和与用户同名的组，-g表示选择用户所在组，如果不写默认会创建一个与用户同名的组。
...
* 设置密码后为新用户添加sudo权限,打开sudoers文件增加如下内容，然后 :w! 强制保存 :q 退出
  ...
  ```
  passwd levi（修改解释器sh为bash，sh为老版本）
...
* ![image-20201102162945818](img/image-20201102162945818.png)
...
* 安装启动
  ...
  - ![image-20201102165428972](img/image-20201102165428972.png)

    <img src="./img/ssh1.png" style="zoom:80%;" />
...
* ssh秘钥（）
  ...
  * 使用方法
    ...
    ```
    1. 在个人计算机中生产秘钥对 ： ssh-keygen  执行以后会在主目录下生成一个.ssh隐藏文件夹,其中包含私钥文件id_rsa和公钥文件id_rsa.pub。
    2. 在服务器主机上创建文件~/.ssh/authorized_keys，将信任的计算机的id_rsa.pub文件内容追加到服务器authorized_keys文件中，并修改其权限为777。
    ```

    ![image-20201102175654642](img/image-20201102175654642.png)
    
    ![image-20201102182554245](img/image-20201102182554245.png)
